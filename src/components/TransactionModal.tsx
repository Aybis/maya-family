...
import { X, Camera, Plus, Calculator, Zap } from 'lucide-react';
...
const TransactionModal: React.FC<TransactionModalProps> = ({ isOpen, onClose, editTransaction }) => {
  ...
  const handleCameraScan = (scanData: any) => {
    setFormData({
      type: scanData.type,
      amount: scanData.amount.toString(),
    ...
    setActiveTab('manual'); // Switch to manual tab to show the filled form
            ...
            {activeTab === 'manual' ? (
              ...
              <div className="text-center py-8">
                <div className={`rounded-2xl p-8 mb-6 transition-colors duration-300 ${
                  ...
                  <Zap className={`h-16 w-16 mx-auto mb-4 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                  <h3 className={`text-lg font-medium mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    AI-Powered Receipt Scanner
                  </h3>
                  <p className={`mb-4 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                    Automatically extract transaction details from your receipt using advanced AI
                  </p>
                  <div className={`text-left space-y-2 text-sm mb-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                    <p>✓ Detects amounts and items</p>
                    <p>✓ Identifies merchant and category</p>
                    <p>✓ Extracts date and payment info</p>
                    <p>✓ Works with receipts in multiple languages</p>
                  </div>
                  <button 
                ...
                <div className={`border rounded-lg p-4 transition-colors duration-300 ${
                  ...
                  <div className={`text-left space-y-2 text-sm ${isDark ? 'text-green-400' : 'text-green-700'}`}>
                    ...
                    <p><strong>Merchant:</strong> Supermarket ABC</p>
